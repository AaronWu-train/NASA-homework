...
\begin{enumerate}
  ...
  \begin{minted}{bash}
    /srv/nfs-share 192.168.128.0/24(rw,async,fsid=0,crossmnt,no_subtree_check,root_squash)
  \end{minted}
  完成後，使用以下指令套用設定：
  \begin{minted}{bash}
    sudo exportfs -arv 
  ...
  \begin{table}[H]
    ...
    \begin{tabular}{
        ...
        *{2}{>{\centering\arraybackslash}p{2cm}} % Write 三欄
        *{1}{>{\centering\arraybackslash}p{2cm}} % Write 三欄
        *{2}{>{\centering\arraybackslash}p{2cm}} % Read 三欄
        *{1}{>{\centering\arraybackslash}p{2cm}} % Read 三欄
      }
      \toprule
