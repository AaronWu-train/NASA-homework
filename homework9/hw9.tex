...
\begin{enumerate}
  ...
  \begin{enumerate}
    \item 根據不同參數，決定使用 \texttt{single} 或 \texttt{parallel} 的方式來執行測試。若要測試 \texttt{single}，可以直接在 \texttt{hpc1} 或 \texttt{hpc2} 上執行 \texttt{nfs\_test.sh}。若要測試 \texttt{parallel}，則需在 server 上執行，並給予 host 清單以及密碼，使得我的 shell script 可以透過 \texttt{parallel-ssh} 和 \texttt{sshpass} 自動登入 \texttt{hpc1} 和 \texttt{hpc2}。
    \item 我使用 \texttt{dd} 指令來將一個大檔案寫入 NFS Server 的資料夾中，並且使用 \texttt{sync} 指令來確保所有的寫入操作都已經完成。
    \item 我使用 \texttt{/usr/bin/time -v} 指令來獲得測試的時間與效能資訊，並將結果寫入檔案。
  \end{enumerate}
