...
\begin{enumerate}
  ...
  \end{minted}
  這個加密方式將一組長度為 10 的 bytes 陣列，去和密鑰做 XOR 運算。因為我們知道原本文字當中一定含有 \texttt{NASA\_HW11\{} 開頭的字串，且密文 XOR 明文會得到密鑰，所以我們可以透過這個已知的字串來破解密鑰。
  \item 在我的 \texttt{decrept\_otp.py} 當中，我首先先用 \texttt{unhexlify} 將密文轉換成 bytes，然後再窮舉 flag 開始的位置，接下來找出每一個開始位置如果明文是 \texttt{NASA\_HW11\{} 時，key 要是多少。
  \item 接著，我在嘗試用這些 key 去解密密文，直到找到一個，解密出的文字包含整個 flag 字串的 regex 的 key。雖然有兩個開始位置算出的 key 都符合 regex，但是其中一個 key 解密出的文字明顯是亂碼，而另一個則是正常的文字。
  \item 我解密出的文字是：
  \begin{minted}{text}
