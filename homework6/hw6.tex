...
\begin{enumerate}
  ...
  \begin{figure}[H]
    ...
    \caption{UFW 設定}
  \end{figure}
  \item 為了驗證 ufw 的設定，我使用以下指令重開 VM，並且使用 ssh 連線到 VM 上。
  \small
  \begin{minted}{bash}
    qemu-system-x86_64 -enable-kvm \
      -cpu host \
      ...
      -boot d \
      -monitor stdio \
      -nic user,hostfwd=tcp::8890-:80,hostfwd=tcp::1102-:22,hostfwd=tcp::8000-:8000 \
      -vnc :2150,password=on
  \end{minted}
  其中，VM 的 port 22 映射到我的工作站 port 1102， port 80 映射到我的工作站 port 8890 的 port 8000 映射到我的工作站 port 8000，並且改掉 vnc 的 port 為 2150 以避開衝突。
  \item 我使用 \texttt{ssh localhost -p 1102} 成功的連線到 VM 上，代表 port 22 防火牆有設定好。
  \item 在 VM 中使用以下指令，在 port 8000 架設一個簡單的 HTTP Server。
  \begin{minted}{bash}
    cd /usr/share/nginx/html
    python3 -m http.server 8000
  \end{minted}
  \item 回到工作站，使用以下指令，確認 port 80 和 port 8000 的防火牆有設定好。
  \begin{minted}{bash}
    curl --max-time 5 localhost:8890 # port 80 of VM is 8890 of workstation
    curl --max-time 5 localhost:8000
  \end{minted}
  \begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{2-1-2.png}
    \caption{確認 port 8000 和 port 80 (8890) 的防火牆有設定好}
  \end{figure}
  \item 我在我的電腦瀏覽器上，以 \texttt{<工作站ip >:8890} 的方式連線到 VM 上的 Nginx 網頁伺服器，其中 8890 是我映射到 VM port 80 的 port，可以看到 Nginx 的預設網頁。
  \begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{2-1-3.png}
    \caption{Nginx 預設網頁}
  \end{figure}
\end{enumerate}
...
\begin{enumerate}
  ...
  \end{minted}
  \item 使用瀏覽器，連上 \texttt{140.112.91.1:8890/$\sim$user1/}，可以看到我剛剛建立的網頁。
  \begin{figure}[H]
    \centering
...
\begin{enumerate}
  ...
  \begin{figure}[H]
    \centering
    \includegraphics[width=0.49\textwidth]{2-5-1.png}
    \includegraphics[width=0.49\textwidth]{2-5-2.png}
  ...
  \begin{figure}[H]
    \centering
    \includegraphics[width=0.49\textwidth]{2-5-3.png}
    \includegraphics[width=0.49\textwidth]{2-5-4.png}
...
\begin{itemize}
  ...
  \begin{figure}[H]
    \centering
    \includegraphics[width=0.49\textwidth]{2-6-1.png}
    \includegraphics[width=0.49\textwidth]{2-6-2.png}
    \caption{Load Balancing Page}
  \end{figure}
  \item 我關閉 Server A 和 Server B，再重新整理網頁，等待 30 秒左右之後，會發現 Server C 的網頁會顯示出來。
  \begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{2-6-3.png}
    \caption{Load Balancing Page}
  \end{figure}
\end{itemize}
